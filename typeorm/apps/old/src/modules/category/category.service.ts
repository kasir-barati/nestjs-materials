...
export class CategoryService {
  ...
  create(createCategoryDto: CreateCategoryDto) {
    return this.categoryRepository.create(createCategoryDto).save();
  }
  ...
  async findAll(categoryId?: string) {
    ...
    const query = this.categoryRepository
      ...
      .leftJoinAndSelect((subQuery: SelectQueryBuilder<Talent>) => {
        return subQuery.select().from(Talent, 'talents').limit(10000);
      }, 'talents')
      .leftJoinAndSelect(
